...
module Services
  ...
  class ConversationHandlerService
    ...
    def extract_ha_actions(result)
      ...
      # Enhanced action extraction with pattern matching and context awareness
      actions.concat(extract_advanced_actions(response_text))

      # Extract contextual actions based on conversation state
      if result[:inner_thoughts]
        actions.concat(extract_contextual_actions(result[:inner_thoughts]))
      end
    ...
    # Advanced action extraction with pattern matching and NLP-like features
    def extract_advanced_actions(response_text)
      actions = []

      # Lighting actions with color and brightness
      if response_text.match(/set.*light.*to (\\w+)/)
      ...
      if response_text.match(/dim.*light.*to (\\d+)%?/) || response_text.match(/set.*brightness.*to (\\d+)%?/)
        brightness = Regexp.last_match(1).to_i
        actions << {
          domain: 'light', service: 'turn_on',
          target: { entity_id: 'light.glitch_cube' },
      ...
      if response_text.match(/what is the.*(temperature|humidity|pressure)/)
        sensor_type = Regexp.last_match(1)
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: "sensor.glitch_cube_#{sensor_type}" }
        }
      end

      # Proactive suggestions (e.g., based on time)
      if response_text.include?('good morning')
        actions << { domain: 'scene', service: 'turn_on', target: { entity_id: 'scene.morning_ambient' } }
      elsif response_text.include?('good night')
        actions << { domain: 'scene', service: 'turn_on', target: { entity_id: 'scene.evening_calm' } }
      end

      actions
    end

    # Extract actions from inner thoughts or contextual cues
    def extract_contextual_actions(inner_thoughts)
      actions = []
      thoughts_text = inner_thoughts.is_a?(String) ? inner_thoughts.downcase : inner_thoughts.to_s.downcase

      # Battery and power management
      if thoughts_text.include?('battery') || thoughts_text.include?('power') || thoughts_text.include?('charge')
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: 'sensor.glitch_cube_battery_level' }
        }
      end

      # User presence and motion detection
      if thoughts_text.include?('user presence') || thoughts_text.include?('motion') || thoughts_text.include?('someone')
        actions << {
          domain: 'binary_sensor', service: 'read',
          target: { entity_id: 'binary_sensor.glitch_cube_motion' }
        }
      end

      # Environmental conditions
      if thoughts_text.include?('temperature') || thoughts_text.include?('hot') || thoughts_text.include?('cold')
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: 'sensor.glitch_cube_temperature' }
        }
      end

      if thoughts_text.include?('humidity') || thoughts_text.include?('dry') || thoughts_text.include?('moist')
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: 'sensor.glitch_cube_humidity' }
        }
      end

      # Lighting considerations
      if thoughts_text.include?('bright') || thoughts_text.include?('dark') || thoughts_text.include?('lighting')
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: 'sensor.glitch_cube_light_level' }
        }
      end

      # Time-based actions
      if thoughts_text.include?('time') || thoughts_text.include?('schedule') || thoughts_text.include?('when')
        actions << {
          domain: 'sensor', service: 'read',
          target: { entity_id: 'sensor.time' }
        }
      end

      # Mood and atmosphere
      if thoughts_text.include?('mood') || thoughts_text.include?('atmosphere') || thoughts_text.include?('vibe')
        actions << {
          domain: 'light', service: 'turn_on',
          target: { entity_id: 'light.glitch_cube' },
          data: { effect: 'mood_responsive' }
        }
      end

      # Music and sound
      if thoughts_text.include?('music') || thoughts_text.include?('sound') || thoughts_text.include?('audio')
        actions << {
          domain: 'media_player', service: 'volume_set',
          target: { entity_id: 'media_player.square_voice' },
          data: { volume_level: 0.3 }
        }
      end

      # Location and GPS
      if thoughts_text.include?('location') || thoughts_text.include?('where') || thoughts_text.include?('position')
        actions << {
          domain: 'device_tracker', service: 'see',
          target: { entity_id: 'device_tracker.glitch_cube' }
        }
      end
