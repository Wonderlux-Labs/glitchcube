  ...
  describe 'Circuit Breaker Behavior' do
      ...
      xit 'transitions to half-open state after timeout', :vcr do
        ...
        allow(Services::Llm::LLMService).to receive(:complete_with_messages) do
          ...
          double('LLMResponse',
                 ...
                 tool_calls?: false,  # Add missing method
                 tool_calls: nil,
    ...
    context 'when Home Assistant service fails' do
      it 'gracefully degrades HA integration features', :vcr do
        allow_any_instance_of(Services::Core::HomeAssistantClient).to receive(:call_service)
          .and_raise(Services::Core::HomeAssistantClient::TimeoutError, 'HA unavailable')
