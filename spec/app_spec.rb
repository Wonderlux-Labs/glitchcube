...
RSpec.describe GlitchCubeApp do
  describe 'GET /' do
    it 'returns welcome message', :vcr do
      get '/'
  ...
  describe 'POST /api/v1/test' do
    context 'with valid message' do
      it 'processes the message through the conversation module', :vcr do
        post '/api/v1/test',
    ...
    context 'with empty body' do
      it 'uses default message', :vcr do
        post '/api/v1/test',
             {}.to_json,
             { 'CONTENT_TYPE' => 'application/json' }
        expect(last_response).to be_ok
  ...
  describe 'POST /api/v1/conversation' do
    it 'processes conversation with AI module', :vcr do
  ...
  describe '404 handling' do
    it 'returns JSON error for unknown routes', :vcr do
      get '/unknown'
