...
RSpec.describe "Home Assistant Conversation Integration", type: :integration do
  ...
  describe "Primary /api/v1/conversation endpoint" do
    context "when called from HA custom conversation agent" do
      it "processes HA conversation through unified endpoint" do
        VCR.use_cassette("ha_primary_conversation") do
      ...
      it "maintains conversation session across HA calls" do
        VCR.use_cassette("ha_conversation_session") do
          # First call
          post '/api/v1/conversation', ha_conversation_payload.to_json, {
            'CONTENT_TYPE' => 'application/json'
          }
      ...
      it "handles HA voice interaction context properly" do
        VCR.use_cassette("ha_voice_interaction") do
          post '/api/v1/conversation', ha_conversation_payload.to_json, {
            'CONTENT_TYPE' => 'application/json'
          }
  ...
  describe "Simplified Architecture - Phase 3.5 COMPLETED" do
    context "webhook endpoints have been removed" do
      it "no longer has complex webhook logic - all removed in consolidation" do
    ...
    it "has successfully consolidated to single conversation endpoint" do
      ...
      VCR.use_cassette("ha_primary_endpoint") do
  ...
  describe "Current HA Custom Agent Analysis" do
    it "documents current custom agent complexity for simplification" do
    ...
    it "shows simplified agent with no bidirectional HA service calls" do
      ...
      sinatra_handles = [
        "TTS via CharacterService.speak()",
        "Lights via LightingTool", 
        "Display via DisplayTool",
        "Music via MusicTool"
      ]
