...
RSpec.describe 'Simple Session Management - Phase 3.5', type: :integration do
  ...
  describe 'New Session Creation' do
    it 'creates new session for each conversation start', :vcr do
  ...
  describe 'Conversation Continuation Logic' do
    it 'uses HA conversation_id to track sessions - same ID = continuation, new ID = new session', :vcr do
  ...
  it 'ends conversation when end_conversation is true', :vcr do
    ...
    # Should include some form of departure/farewell language
    expect(response_data['data']['response']).to match(/(?i)(goodbye|bye|farewell|see you|leaving|come back|don't forget)/)
  end

  describe 'Feedback Hooks Integration' do
    it 'provides feedback state information in response', :vcr do
      session_id = "voice_#{Time.now.to_i}_#{rand(1000..9999)}"
      post '/api/v1/conversation', {
  ...
  describe 'Voice Interaction Context' do
    it 'handles voice-specific context correctly', :vcr do
      session_id = "voice_#{Time.now.to_i}_#{rand(1000..9999)}"
      post '/api/v1/conversation', {
