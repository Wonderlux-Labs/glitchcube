...
RSpec.describe 'Conversation Continuation Logic', type: :integration do
  ...
  describe 'Structured Output Parsing' do
    context 'with properly formatted LLM responses' do
      it 'correctly parses continue_conversation flag when false (ending)', :vcr do
      ...
      it 'correctly parses continue_conversation flag when true (continuing)', :vcr do
        post '/api/v1/conversation', {
  ...
  describe 'Edge Cases' do
    it 'handles empty user input gracefully', :vcr do
      post '/api/v1/conversation', {
        message: '',
        context: {
    ...
    it 'respects explicit user termination requests', :vcr do
      ...
      expect(first_response['data']).not_to be_nil
  ...
  describe 'Concurrent Conversations' do
    it 'maintains separate context for simultaneous sessions', :vcr do
      ...
      expect(response_a1['data']['response']).to include_any_of(%w[temperature weather degrees])
  ...
  describe 'Proactive Conversations' do
    it 'handles automation-triggered conversations that expect user response', :vcr do
