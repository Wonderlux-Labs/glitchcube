...
- **Key Services**:
  - `ConversationModule`: Simple interface for conversation processing
  - `ConversationFlowManager`: Central conversation orchestrator
  - `LLMService`: OpenRouter API integration with circuit breaker protection
  - `HomeAssistantClient`: Hardware control interface
  - `ToolExecutor`: Function calling for LLM tools
  - **Modular Conversation Services**: Specialized services for different aspects of conversation processing (LLM interaction, tool execution, response processing, state management)
