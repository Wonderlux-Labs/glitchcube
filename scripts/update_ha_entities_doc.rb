...
class HAEntitiesDocUpdater
  ...
  def generate_documentation(entities_by_domain, states, used_entities)
    ...
    used_entities.each do |entity_id, info|
      doc << if info[:exists]
               "- ✅ **#{entity_id}** - #{info[:friendly_name]} (State: #{info[:state]})"
             else
               "- ❌ **#{entity_id}** - (Missing - needs configuration)"
             end
    end
    ...
    entities_by_domain.each do |domain, entities|
      ...
      entities.each do |entity|
        ...
        doc << if entity_id.include?('weather') || entity_id.include?('camera') || entity_id.include?('vision')
        ...
        next unless domain == 'sensor' && entity_id.include?('weather')

        attributes = entity['attributes'] || {}
        doc << '  - Has weather_data attribute' if attributes['weather_data']
      end
  ...
  def print_summary(entities_by_domain)
    ...
    %w[weather camera sensor input_text media_player].each do |domain|
      puts "  #{domain}: #{entities_by_domain[domain].count} entities" if entities_by_domain[domain]
    end
  end
end
