...
class MusicTool
      ...
      if verbose
        media_players.each do |player|
          ...
          if player['state'] != 'unavailable'
            ...
            features = []
            features << "play/pause" if (supported_features & 1) != 0 # SUPPORT_PAUSE
            features << "volume" if (supported_features & 4) != 0     # SUPPORT_VOLUME_SET
            features << "seek" if (supported_features & 8) != 0       # SUPPORT_SEEK
            features << "next/prev" if (supported_features & 16) != 0 # SUPPORT_NEXT_TRACK
            features << "turn_on/off" if (supported_features & 128) != 0 # SUPPORT_TURN_ON
  ...
  def self.set_volume(client, params)
    ...
    volume_level = volume_level / 100.0 if volume_level > 1.0 # Convert percentage to decimal
