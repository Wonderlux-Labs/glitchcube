...
class SpeechTool < BaseTool
  ...
  def self.list_voices(verbose: false)
    ...
    begin
      ...
      tts_services = services.select { |domain, _| domain == 'tts' }
      ...
      tts_services.each do |domain, service_info|
        service_info.each do |service_name, service_details|
          ...
          if verbose && service_details['fields']
  ...
  def self.announce(message:, priority: 'normal')
    entity = case priority
             when 'high'
               DEFAULT_ENTITY  # Use main speaker for high priority
             when 'low'  
               DEFAULT_ENTITY  # Could use different entity for background announcements
             else
               DEFAULT_ENTITY
             end
