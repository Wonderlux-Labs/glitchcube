...
class CameraTool
  ...
  def self.list_available_cameras(client, params)
    ...
    begin
      ...
      if verbose
        ...
        motion_entities = states.select { |s| 
  ...
  def self.get_motion_detection_status(client, params)
    result = []
    ...
    begin
      ...
      if sensitivity
        result << "Motion sensitivity: #{sensitivity['state']}"
      end
  ...
  def self.analyze_camera_scene(client, params)
    ...
    begin
      ...
      snapshot_result = client.take_snapshot(entity_id: entity_id)
