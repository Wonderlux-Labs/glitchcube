...
module Services
  ...
  class UnifiedLoggerService
    class << self
      ...
      def api_call(service:, method: 'POST', endpoint:, status: nil, duration: nil, error: nil, **metadata)
        ...
        level = error ? :error : (status && status >= 400 ? :warn : :info)
      ...
      def create_unified_logger
        begin
          ...
          $stderr.puts "Failed to setup file logger: #{e.message}. Using STDOUT."
      ...
      def log(level, message, **metadata)
        ...
        if Cube::Settings.development? || Cube::Settings.log_level == Logger::DEBUG
      ...
      def format_console_message(level, message, **metadata)
        ...
        if metadata[:duration_ms]
          formatted += " (#{metadata[:duration_ms]}ms)"
        end
        
        if metadata[:status]
          formatted += " [#{metadata[:status]}]"
        end
      ...
      def ensure_log_directory
        ...
        unless File.writable?(dir)
          $stderr.puts "Log directory #{dir} is not writable. Attempting to fix permissions..."
          FileUtils.chmod(0o755, dir)
        end
      rescue StandardError => e
        $stderr.puts "Failed to create log directory: #{e.message}"
        raise e
      end
