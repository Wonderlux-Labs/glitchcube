...
module Services
  ...
  class ConversationFeedbackService
    # LED ring entity
    SPEAKER_LED_RING = 'light.cube_voice_ring'
    ...
    def set_solid_color(rgb_color, brightness, description)
      result = @home_assistant.call_service('light', 'turn_on', {
                                              entity_id: SPEAKER_LED_RING,
                                              rgb_color: rgb_color,
                                              brightness: brightness,
                                              transition: 0.5
      ...
      puts "🔵 HA call_service result: #{result.inspect}" if GlitchCube.config.development?
      log_feedback('solid', description, rgb_color, brightness)
      true
    rescue StandardError => e
      puts "❌ HA call_service error: #{e.class} - #{e.message}" if GlitchCube.config.development?
      log_error('solid', e)
      false
    end

    # Pulsing effect using brightness transitions
    def set_pulsing_effect(rgb_color, brightness, description:, slow: true)
      # Set initial color
