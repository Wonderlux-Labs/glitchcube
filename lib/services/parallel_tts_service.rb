...
module Services
  ...
  class ParallelTTSService < TTSService
    ...
    def speak_race(
      message,
      providers: %i[cloud google piper],
      **options
    )
    ...
    def speak_cascade(
      message,
      providers: %i[cloud google piper elevenlabs],
      **options
    )
      return false if message.nil? || message.strip.empty?
      ...
      providers.each_with_index do |provider, index|
        ...
        sleep(index * 0.5) if index.positive?
    ...
    def speak_intelligent(
      ...
      **options
      ...
      when :critical
        ...
        speak_redundant(message, providers: %i[cloud google piper], **options)
      when :fast
        # Use race for fastest response
        speak_race(message, providers: %i[cloud google], **options)
      when :reliable
        # Use cascade with all providers
        speak_cascade(message, providers: %i[cloud google piper elevenlabs], **options)
      else
    ...
    def wait_for_first_success(futures, providers, timeout: 5)
      ...
      while Time.now < end_time
        futures.each_with_index do |future, index|
          ...
          result = future.value
          next if result.is_a?(Hash) && !result[:success]
        ...
        break if futures.all? { |f| f.rejected? || (f.fulfilled? && f.value.is_a?(Hash) && !f.value[:success]) }
