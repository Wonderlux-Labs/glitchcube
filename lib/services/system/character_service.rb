...
module Services
  class CharacterService
    ...
    def initialize(character: :default, home_assistant: nil)
      ...
      begin
        @persona = Personas::BasePersona.create(@character.to_s) if Personas::BasePersona.persona_exists?(@character.to_s)
      rescue StandardError
        @persona = nil
      end
    end

    # Bridge method to get persona tools
    def get_tools
      return @persona.tool_schemas if @persona

      # Fall back to character config
      @character_config[:tools] || []
    end

    # Bridge method to get persona prompt
    def get_system_prompt(_context = {})
      return @persona.generate_system_prompt if @persona

      # Fall back to basic prompt
      "You are #{@character_config[:name]}: #{@character_config[:description]}"
    end
