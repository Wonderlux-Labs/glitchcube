...
begin
    ...
    if entity['attributes']
      entity['attributes'].each do |key, value|
        if value.is_a?(Array) && key == 'supported_languages'
          ...
          if value.length > 0
            puts "      Examples: #{value.first(5).join(', ')}#{value.length > 5 ? '...' : ''}"
          end
        else
          puts "    #{key}: #{value}"
        end
  ...
  begin
    ...
    if services_response && services_response.is_a?(Array)
      ...
      if tts_services.any?
        ...
        tts_services.first['services'].each do |service_name, service_info|
          ...
          if service_info['description']
            puts "    Description: #{service_info['description']}"
          end
          if service_info['fields']
            puts "    Parameters:"
